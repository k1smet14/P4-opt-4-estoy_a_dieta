...
def create_multi_dataloader(
    ...
    """
    # Data Setup
    train_dataset, val_dataset, test_dataset = get_multi_tico_dataset(
        data_path=config["DATA_PATH"],
        dataset_name=config["DATASET"],
        img_size=config["IMG_SIZE"],
        val_ratio=config["VAL_RATIO"],
...
def get_multi_tico_dataset(
    ...
    # preprocessing policies
    t_transform_train = getattr(
        __import__("src.augmentation.policies", fromlist=[""]),
        transform_train,
    )(dataset=dataset_name, img_size=224, **transform_train_params)
    s_transform_train = getattr(
        __import__("src.augmentation.policies", fromlist=[""]),
        transform_train,
    )(dataset=dataset_name, img_size=img_size, **transform_train_params)
    ...
    train_dataset = MultiSizeDataset(root=train_path, t_transform=t_transform_train,s_transform=s_transform_train)
    val_dataset = ImageFolder(root=val_path, transform=transform_test)
    test_dataset = ImageFolder(root=test_path, transform=transform_test)

    return train_dataset, val_dataset, test_dataset

class MultiSizeDataset(data.Dataset):
    def __init__(self, root,t_transform,s_transform):
        self.classes = ['Battery', 'Clothing', 'Glass', 'Metal', 'Paper', 'Paperpack', 'Plastic', 'Plasticbag', 'Styrofoam']
        clss = glob.glob(os.path.join(root,'*'))
        self.imgs,self.labels = [],[]
        self.t_transform = t_transform
        self.s_transform = s_transform
        for c in clss:
            c_str = c.replace(root+'/','')
            imgs = glob.glob(os.path.join(c,'*'))
            self.imgs.extend(imgs)
            self.labels.extend([self.classes.index(c_str)] * len(imgs))
    ...
    def __getitem__(self, idx):
        ...
        return t_img,s_img,torch.LongTensor([label])
