...
class MuxInvertedResidualGenerator(GeneratorAbstract):
    ...
    def out_channel(self) -> int:
        ...
        return int(self.in_channel * self.width_multiply)
    ...
    def __call__(self, repeat: int = 1):
        ...
        """
        module = []
        k, s, ex_ratio, se_ratio, scales, act_name = self.args  # c is equivalent as self.out_channel
...
class InvertedResidualGenerator(GeneratorAbstract):
    ...
    def out_channel(self) -> int:
        """Get out channel size."""
        return int(self.args[2] * self.width_multiply)
    ...
    def __call__(self, repeat: int = 1):
        ...
        """
        module = []
        k, s, _, ex_ratio,se_ratio, act_name = self.args  # c is equivalent as self.out_channel
        inp, oup = self.in_channel, self.out_channel
...
class SpaceToChannelGenerator(GeneratorAbstract):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def out_channel(self) -> int:
        """Get out channel size."""
        return int(self.in_channel*(4**self.repeat))
