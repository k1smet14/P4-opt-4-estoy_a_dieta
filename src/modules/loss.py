...
class F1_CE_Loss(nn.Module):
    def __init__(self, f1rate=0.4 ,classes=9):
        super().__init__()
        self.rate = f1rate
        self.f1 = F1Loss(classes=classes)
    ...
    def forward(self, preds, truth):
        ...
        return f1*self.rate + ce*(1-self.rate)
...
class KD_loss(nn.Module):
    def __init__(self, s_loss='CE', temperature=20, alpha=0.3):
        ...
        if s_loss=='f1focal':
            ...
            self.s_loss = F1_CE_Loss(f1rate=0.6)
...
class TKD_loss(nn.Module):
    ...
    def forward(self, t_preds, s_preds, truth):
        ttp = (torch.argmax(t_preds,dim=-1) == truth) * 1.
        ttp = ttp.unsqueeze(-1)
        dist_loss = self.KLDiv(F.log_softmax(s_preds/self.T, dim=1), F.softmax(t_preds/self.T, dim=1)) * ttp
        stud_loss = self.s_loss(s_preds, truth)
        return dist_loss.sum(1).mean()*(self.alpha*self.T*self.T)  +  stud_loss*(1.-self.alpha)
...
class SKD_loss(nn.Module):
    ...
    def forward(self, t_preds, s_preds, truth):
        t_preds = F.layer_norm(t_preds, torch.Size((t_preds.size(1),)), None, None, 1e-7)
        s_preds = F.layer_norm(s_preds, torch.Size((s_preds.size(1),)), None, None, 1e-7)
        dist_loss = self.KLDiv(F.log_softmax(s_preds/self.T, dim=1), F.softmax(t_preds/self.T, dim=1))
        stud_loss = self.s_loss(s_preds, truth)
        return dist_loss*(self.alpha*self.T*self.T)  +  stud_loss*(1.-self.alpha)