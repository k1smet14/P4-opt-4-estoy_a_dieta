...
from src.utils.data import cutmix
from src.utils.torch_utils import save_model
from src.loss import KD_loss, F1_Focal_Loss
...
class TorchTrainer:
    ...
    def train(
        ...
        for epoch in range(n_epoch):
            ...
            for batch, (data, labels) in pbar:
                ...
                r = np.random.rand(1)
                if cutmix_prob > 0:
                    data, lam, target_a, target_b = cutmix(data, labels, beta, cutmix_prob, r)
            ...
            wandb.log({'val_loss' : test_loss, 'val_f1' : test_f1, 'val_acc' : test_acc})
    ...
    def train_kd(
        ...
        wandb.init(project="Optimization")
        wandb.watch(self.model)
        kd_loss_fn = KD_loss(s_loss='f1focal', temperature=20, alpha=0.7)
        val_loss_fn = F1_Focal_Loss(f1_rate=0.6, gamma=2.0)
        for epoch in range(n_epoch):
            running_loss, correct, total = 0.0, 0, 0
            preds, gt = [], []
            pbar = tqdm(enumerate(train_dataloader), total=len(train_dataloader))
            ...
            self.model.train()
            self.t_model.eval()
            for batch, (t_imgs, s_imgs, labels) in pbar:
                t_imgs = t_imgs.to(self.device).float()
                s_imgs = s_imgs.to(self.device).float()
                labels = labels.to(self.device)
                
                with torch.no_grad():
                    t_outputs = self.t_model(t_imgs)
                outputs = self.model(s_imgs)
                loss = kd_loss_fn(t_outputs, outputs, labels)
