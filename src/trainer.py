...
from src.modules.loss import KD_loss,TKD_loss, SKD_loss
...
def _get_len_label_from_dataset(dataset: Dataset) -> int:
    ...
    """
    if isinstance(dataset, torchvision.datasets.ImageFolder) or isinstance(dataset, torchvision.datasets.vision.VisionDataset) or isinstance(dataset, torch.utils.data.Dataset):
...
class TorchKDTrainer:
    ...
    def __init__(
        ...
        if kd_method == 'true_kd':
            self.kd_criterion = TKD_loss(s_loss='f1focal', temperature=20, alpha=0.3)
        elif kd_method == 'skd':
            self.kd_criterion = SKD_loss(s_loss='f1focal', temperature=20, alpha=0.3)
        else:
            self.kd_criterion = KD_loss(s_loss='f1focal', temperature=20, alpha=0.3)
    ...
    def train(
        ...
        for epoch in range(n_epoch):
            ...
            for batch, (data, labels) in pbar:
                ...
                s_output = self.s_model(data)
                loss = self.kd_criterion(t_output, s_output, labels)
                self.optimizer.zero_grad()
            ...
            _, test_f1, test_acc = self.test(
                model=self.s_model, test_dataloader=val_dataloader
            )
    ...
    def multi_train(
        ...
        for epoch in range(n_epoch):
            ...
            for batch, (t_data,s_data, labels) in pbar:
                t_data, s_data, labels = t_data.to(self.device), s_data.to(self.device), labels[:,0].to(self.device)
                with torch.no_grad():
                    t_output = self.t_model(t_data)
                
                
                s_output = self.s_model(s_data)
                loss = self.kd_criterion(t_output, s_output, labels)
